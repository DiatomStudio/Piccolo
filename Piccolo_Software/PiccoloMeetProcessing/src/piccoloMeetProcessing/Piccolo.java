...
public class Piccolo extends PGraphics {
	...
	//Matrix variables  	
	  ...
	  float flatRes = 0.5f;// steps per mm around curves 
...
public Piccolo(float _w, float _h, float _d){
	bezierDetail(50);
}
...
if(inString.startsWith("setZ")){
 ...
 // pressureKnob.setValue(zVal); // add soem type of callback?
  ...
  public void removeOcclusions(PGraphics occlusionCanvas){
	 ...
	 * @param theParent
	 */
	public Piccolo(PApplet theParent) {
		myParent = theParent;
		welcome();
	}
